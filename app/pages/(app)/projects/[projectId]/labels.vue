...
const error = ref("");
const editingLabel = ref<{ id: string; name: string; color: string } | null>(null);
...
const colors = ["#6366f1", "#ef4444", "#f59e0b", "#22c55e", "#3b82f6", "#ec4899", "#14b8a6", "#a855f7"];
...
async function save(event: FormSubmitEvent<LabelFormData>) {
  ...
  try {
    if (editingLabel.value) {
      await $fetch(`/api/projects/${projectId}/labels/${editingLabel.value.id}`, {
        method: "PATCH",
        body: event.data,
      });
    } else {
...
<template>
  <UContainer class="py-6 max-w-2xl">
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <UButton icon="i-lucide-plus" label="New label" size="sm" @click="openCreate" />
    </div>

    <UCard v-if="showForm" class="mb-4">
      <UForm :schema="labelSchema" :state="formState" class="space-y-4" @submit="save">
        ...
        <div class="flex gap-2">
          <UButton type="submit" :label="editingLabel ? 'Update' : 'Create'" :loading="saving" />
    ...
    <UCard :ui="{ body: 'p-0' }">
      <div v-for="label in (labels as any[])" :key="label.id" class="flex items-center gap-3 px-4 py-3 border-b border-(--ui-border) last:border-b-0">
        ...
        <UButton icon="i-lucide-pencil" size="2xs" color="neutral" variant="ghost" @click="openEdit(label)" />
        <UButton icon="i-lucide-trash-2" size="2xs" color="error" variant="ghost" @click="remove(label.id)" />
      </div>
      <div v-if="(labels as any[]).length === 0" class="px-4 py-8 text-center text-sm text-(--ui-text-muted)">
        No labels yet
      </div>
    </UCard>
  </UContainer>
