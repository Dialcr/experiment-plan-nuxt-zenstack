...
async function save(event: FormSubmitEvent<SprintForm>) {
  ...
  try {
    ...
    if (event.data.start_date) body.start_date = new Date(event.data.start_date).toISOString();
...
function statusIcon(status: string) {
  switch (status) {
    case "ACTIVE": return "i-lucide-play-circle";
...
<template>
  <UContainer class="py-6 max-w-3xl">
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <UButton icon="i-lucide-plus" label="New sprint" size="sm" @click="openCreate" />
    </div>

    <UCard v-if="showForm" class="mb-4">
      <UForm :schema="sprintSchema" :state="formState" class="space-y-4" @submit="save">
        <UFormField label="Sprint name" name="name" required>
          <UInput v-model="formState.name" class="w-full" autofocus placeholder="Sprint 1" />
        </UFormField>
        ...
        <div class="flex gap-2">
          <UButton type="submit" :label="editingSprint ? 'Update' : 'Create'" :loading="saving" />
    ...
    <template v-for="(group, key) in groupedSprints" :key="key">
      <div v-if="group.length > 0" class="mb-6">
        <h4 class="text-sm font-semibold text-(--ui-text-muted) uppercase tracking-wider mb-3 flex items-center gap-2">
          <UIcon :name="statusIcon(key)" class="size-4" />
          {{ key === "ACTIVE" ? "Active" : key === "PLANNED" ? "Upcoming" : "Completed" }}
          <UBadge size="xs" color="neutral" variant="subtle">{{ group.length }}</UBadge>
        </h4>
        <div class="space-y-2">
          <UCard
            ...
            <div class="flex items-center gap-3">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <UBadge v-if="sprint.status === 'ACTIVE'" size="xs" color="success" variant="subtle">Active</UBadge>
                </div>
                <p v-if="sprint.description" class="text-xs text-(--ui-text-muted) truncate mt-0.5">{{ sprint.description }}</p>
              ...
              <UDropdownMenu :items="[
                sprint.status === 'PLANNED' ? { label: 'Start sprint', icon: 'i-lucide-play', onSelect: () => updateStatus(sprint.id, 'ACTIVE') } : null,
                sprint.status === 'ACTIVE' ? { label: 'Complete sprint', icon: 'i-lucide-check', onSelect: () => updateStatus(sprint.id, 'COMPLETED') } : null,
                sprint.status === 'COMPLETED' ? { label: 'Reopen sprint', icon: 'i-lucide-rotate-ccw', onSelect: () => updateStatus(sprint.id, 'ACTIVE') } : null,
                { type: 'separator' } as any,
                { label: 'Edit', icon: 'i-lucide-pencil', onSelect: (e: Event) => { e.stopPropagation(); openEdit(sprint); } },
                { label: 'Delete', icon: 'i-lucide-trash-2', color: 'error' as any, onSelect: async (e: Event) => { e.stopPropagation(); deletingId = sprint.id; await remove(sprint.id); } },
              ].filter(Boolean) as any[]"
                :content="{ align: 'end' }"
              >
                <UButton icon="i-lucide-more-horizontal" size="2xs" color="neutral" variant="ghost" @click.stop />
              </UDropdownMenu>
            </div>
          </UCard>
        </div>
